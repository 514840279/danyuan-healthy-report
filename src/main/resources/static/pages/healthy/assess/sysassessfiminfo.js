window.operateEvents = {
	...
	'click #clickA ': function (e, value, row, index) {
		var url="/sysAssessFimInfo/detail/"+row['uuid'];
		loadPage(url);
	},
	// 下载
	'click #clickB ': function (e, value, row, index) {
		var url="/sysAssessFimInfo/exportOne/"+row['uuid'];
		window.open(url);
	},
	// 导出
	'click #clickC ': function (e, value, row, index) {
		var url ="/sysAssessFimInfo/exportModal/"+row['uuid'];
		loadPage(url);
	},
	// 删除
	'click #clickD': function (e, value, row, index) {
		...
		bootbox.confirm({
			...
			callback : function(result) {
				if (result) {
					// 删除
					var url = "/sysAssessFimInfo/delete";	
					ajaxPost(url, row, refreshSysAssessFimInfo);	
				}
			}
		});
	}
...
function init() {
	// 条件查询 
	$("#make_sure_search_healthy_assess_sysAssessFimInfo_button_id").bind("click",function(){ 
		$('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable("refreshOptions",{pageNumber:1});
	})
	
	// 弹出编辑窗口
	$('#addnew_healthy_assess_sysAssessFimInfo').click(function() {
		$("#healthy_assess_sysAssessFimInfo_uuid").val("");
		$("#healthy_assess_sysAssessFimInfo_deleteFlag").val("");
		$("#healthy_assess_sysAssessFimInfo_discription").val("");
		$("#healthy_assess_sysAssessFimInfo_assessUuid").val("");
		$("#healthy_assess_sysAssessFimInfo_score").val("");
		$("#healthy_assess_sysAssessFimInfo_name").val("");
		$("#healthy_assess_sysAssessFimInfo_orderNum").val("");
		$('#healthy_assess_sysAssessFimInfo_modal').modal({
			show:true,
		});
	});
	// 反填数据并弹出编辑窗口
	$('#editold_healthy_assess_sysAssessFimInfo').click(function() {
		var data = $('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable('getAllSelections');
		if(data.length == 0||data.length >1){
			alert("必须选中一条数据");
		}else if(data.length > 0){
			$("#healthy_assess_sysAssessFimInfo_uuid").val(data[0].uuid);
			$("#healthy_assess_sysAssessFimInfo_deleteFlag").val(data[0].deleteFlag);
			$("#healthy_assess_sysAssessFimInfo_discription").val(data[0].discription);
			$("#healthy_assess_sysAssessFimInfo_assessUuid").val(data[0].assessUuid);
			$("#healthy_assess_sysAssessFimInfo_score").val(data[0].score);
			$("#healthy_assess_sysAssessFimInfo_name").val(data[0].name);
			$("#healthy_assess_sysAssessFimInfo_orderNum").val(data[0].orderNum);
	...
	// 删除记录
	$('#deleteold_healthy_assess_sysAssessFimInfo').click(function() {
		var data = $('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable('getAllSelections');
		if(data.length == 0){
			alert("先选中一条数据");
		}else if(data.length>0){
			...
			callback : function(result) {
					if (result) {
						var url = "/sysAssessFimInfo/deleteAll";
						var param={list:data};
						ajaxPost(url, param, refreshSysAssessFimInfo);
					}
				}
			});
	...
	// 更新数据
	$('#healthy_assess_sysAssessFimInfo_save_button').click(function() {
		...
		ajaxPost(url, info, refreshSysAssessFimInfo );
		$('#healthy_assess_sysAssessFimInfo_modal').modal("hide");
	});

	// bootstrap table 初始化
	$('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable({
		url : "/sysAssessFimInfo/page",
		dataType : "json",
		toolbar : '#healthy_assess_sysAssessFimInfo_toolbar', // 工具按钮用哪个容器
		...
		queryParams: function queryParams(params) {  
			var param = {  
				...
				info:{
					assessUuid: $("#search_healthy_assess_sysAssessFimInfo_assessUuid").val()==""?null:$("#search_healthy_assess_sysAssessFimInfo_assessUuid").val(),
				}
			}; 
			return param;
		},
		columns : [ 
			...
			{title : '字段排序',	field : 'orderNum','width':150,align : 'right',sortable : true,valign : 'middle',switchable:true,visible:true},
...
// 表格重载 事件
function refreshSysAssessFimInfo(){
	$('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable('refresh');
}

// 窗口大小改变时 重设表头
$(window).resize(function() {
	$('#healthy_assess_sysAssessFimInfo_datagrid').bootstrapTable('resetView');
});

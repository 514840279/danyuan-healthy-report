window.operateEvents = {
	...
	'click #clickA ': function (e, value, row, index) {
		var url="/sysAssessAsiaInfo/detail/"+row['uuid'];
		loadPage(url);
	},
	// 下载
	'click #clickB ': function (e, value, row, index) {
		var url="/sysAssessAsiaInfo/exportOne/"+row['uuid'];
		window.open(url);
	},
	// 导出
	'click #clickC ': function (e, value, row, index) {
		var url ="/sysAssessAsiaInfo/exportModal/"+row['uuid'];
		loadPage(url);
	},
	// 删除
	'click #clickD': function (e, value, row, index) {
		...
		bootbox.confirm({
			...
			callback : function(result) {
				if (result) {
					// 删除
					var url = "/sysAssessAsiaInfo/delete";	
					ajaxPost(url, row, refreshSysAssessAsiaInfo);	
				}
			}
		});
	}
...
function init() {
	// 条件查询 
	$("#make_sure_search_healthy_assess_sysAssessAsiaInfo_button_id").bind("click",function(){ 
		$('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable("refreshOptions",{pageNumber:1});
	})
	
	// 弹出编辑窗口
	$('#addnew_healthy_assess_sysAssessAsiaInfo').click(function() {
		$("#healthy_assess_sysAssessAsiaInfo_uuid").val("");
		$("#healthy_assess_sysAssessAsiaInfo_deleteFlag").val("");
		$("#healthy_assess_sysAssessAsiaInfo_discription").val("");
		$("#healthy_assess_sysAssessAsiaInfo_jibie").val("");
		$("#healthy_assess_sysAssessAsiaInfo_assessUuid").val("");
		$("#healthy_assess_sysAssessAsiaInfo_biaoxian").val("");
		$('#healthy_assess_sysAssessAsiaInfo_modal').modal({
			show:true,
		});
	});
	// 反填数据并弹出编辑窗口
	$('#editold_healthy_assess_sysAssessAsiaInfo').click(function() {
		var data = $('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable('getAllSelections');
		if(data.length == 0||data.length >1){
			alert("必须选中一条数据");
		}else if(data.length > 0){
			$("#healthy_assess_sysAssessAsiaInfo_uuid").val(data[0].uuid);
			$("#healthy_assess_sysAssessAsiaInfo_deleteFlag").val(data[0].deleteFlag);
			$("#healthy_assess_sysAssessAsiaInfo_discription").val(data[0].discription);
			$("#healthy_assess_sysAssessAsiaInfo_jibie").val(data[0].jibie);
			$("#healthy_assess_sysAssessAsiaInfo_assessUuid").val(data[0].assessUuid);
			$("#healthy_assess_sysAssessAsiaInfo_biaoxian").val(data[0].biaoxian);
	...
	// 删除记录
	$('#deleteold_healthy_assess_sysAssessAsiaInfo').click(function() {
		var data = $('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable('getAllSelections');
		if(data.length == 0){
			alert("先选中一条数据");
		}else if(data.length>0){
			...
			callback : function(result) {
					if (result) {
						var url = "/sysAssessAsiaInfo/deleteAll";
						var param={list:data};
						ajaxPost(url, param, refreshSysAssessAsiaInfo);
					}
				}
			});
	...
	// 更新数据
	$('#healthy_assess_sysAssessAsiaInfo_save_button').click(function() {
		...
		ajaxPost(url, info, refreshSysAssessAsiaInfo );
		$('#healthy_assess_sysAssessAsiaInfo_modal').modal("hide");
	});

	// bootstrap table 初始化
	$('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable({
		url : "/sysAssessAsiaInfo/page",
		dataType : "json",
		toolbar : '#healthy_assess_sysAssessAsiaInfo_toolbar', // 工具按钮用哪个容器
		...
		queryParams: function queryParams(params) {  
			var param = {  
				...
				info:{
					assessUuid: $("#search_healthy_assess_sysAssessAsiaInfo_assessUuid").val()==""?null:$("#search_healthy_assess_sysAssessAsiaInfo_assessUuid").val(),
				}
			}; 
			return param;
		},
		columns : [ 
			...
			{title : '临床表现',	field : 'biaoxian','width':150,align : 'left',sortable : true,valign : 'middle',switchable:true,visible:true},
...
// 表格重载 事件
function refreshSysAssessAsiaInfo(){
	$('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable('refresh');
}

// 窗口大小改变时 重设表头
$(window).resize(function() {
	$('#healthy_assess_sysAssessAsiaInfo_datagrid').bootstrapTable('resetView');
});
